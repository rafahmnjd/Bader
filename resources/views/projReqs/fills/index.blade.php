        ...
        <div class="card">
            <div class="card-header">
                <h5 class="card-title">{{__('Manage Fills')}}
                    <a id="top-plus" href="{{route('projReqs.fills.create',$projReq)}}"
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <th scope="col">{{ __('projReq') }}</th>
                                <th scope="col">{{ __('quantity') }}</th>
                                <th scope="col">{{ __('state') }}</th>
                                <th scope="col">{{ __('unite cost') }}</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $fill->projReq->name_ar}}</td>
                                <td>{{ $fill->quantity }}</td>
                                <td>{{ $fill->cost }}</td>

                                <td>{{ $fill->state }}
                                    @if($fill->state != "completed")
                                    <form action="{{ route('projReqs.fills.close', $fill->id) }}" method="POST">
                                        @csrf
                                        @method('put')
                                    ...
                                    <div class="btn-group-justified">
                                        ...
                                        <div class="btn-group">
                                            <a class="btn btn-outline-warning rounded-circle"
                                                href="{{ route('projReqs.fills.edit', $fill->id) }}">
                                                <i class="zmdi zmdi-edit"></i>
                                            </a>
                                        </div>
                                        <div class="btn-group">
                                            <form action="{{ route('projReqs.fills.destroy', $fill->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
